    ...
    for (const referral of pendingReferrals) {
      try {
        ...
        if (loginCount >= REQUIRED_LOGINS) {
          ...
          console.log(`✅ Validating referral: ${referral.referredUsername} (${loginCount} logins)`);

          const result = await validateReferral(new ObjectId(referral._id));

          if (result) {
            validated++;
            console.log(`   💰 Rewards distributed to ${referral.referrerUsername}`);
          } else {
            errors++;
          ...
          console.log(`❌ Invalidating referral: ${referral.referredUsername} (only ${loginCount} logins)`);
